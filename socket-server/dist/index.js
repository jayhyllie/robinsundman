...
async function buildSessionState(sessionId, forAdmin = false) {
    ...
    const currentQuestion = currentQQ
        ? {
            ...
            type: currentQQ.question.type,
            timeLimitSec: currentQQ.timeLimitSec,
            options: currentQQ.question.options.map((o) => ({
                id: o.id,
                letter: o.letter,
                labelSv: o.labelSv,
                labelEn: o.labelEn,
...
async function rebuildActiveLeaderboardsFromAnswers() {
    ...
    const companyBestByQuestion = new Map();
    for (const answer of answers) {
        ...
        companyBestByQuestion.set(companyKey, Math.max(companyBestByQuestion.get(companyKey) ?? 0, answer.points));
    }
...
const companyQuestionBest = new Map();
async function updateLeaderboardScores(companyId, playerName, playerPoints, quizQuestionId) {
    await ensureActiveLeaderboardPeriods();
    const periods = await prisma.leaderboardPeriod.findMany({
        where: { isActive: true },
    });
...
async function revealQuestion(sessionId) {
    ...
    if (qq.question.type === "MULTIPLE_CHOICE") {
        ...
        for (const answer of gradedAnswers) {
            await updateLeaderboardScores(answer.participant.companyId, answer.participant.playerName, answer.points, qq.id);
        }
        ...
        io.to(`admin:${sessionId}`).emit("free_text_submissions", submissions.map((s) => ({
            answerId: s.id,
...
function clearRoomTimers(sessionId) {
    const room = roomStates.get(sessionId);
    if (!room)
        return;
    if (room.revealTimer) {
...
function scheduleAutoComplete(sessionId) {
    const room = ensureRoom(sessionId);
    if (room.endTimer)
        clearTimeout(room.endTimer);
...
function isLastQuizQuestion(questions, current) {
...
io.on("connection", (socket) => {
    ...
    socket.on("join_admin", async (payload) => {
        const parsed = z
            .object({
            sessionId: z.string(),
            secret: z.string(),
    ...
    socket.on("award_free_text", async (payload) => {
        if (!socket.data.isAdmin)
            return;
        const parsed = z
            .object({
        ...
        for (const award of parsed.data.awards) {
            ...
            if (award.isCorrect && award.points > 0) {
                ...
                await updateLeaderboardScores(answer.participant.companyId, answer.participant.playerName, award.points, answer.quizQuestionId);
            }
        }
        await prisma.quizSession.update({
            where: { id: parsed.data.sessionId },
